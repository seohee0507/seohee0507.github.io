...
> GraphQL 스키마(Query 정의) &rightarrow; Query export(질의) &rightarrow; Gatsby 요청/응답 &rightarrow; 응답 데이터를 Props(data 키)로 전달
...
## Gatsby Link API
- 경로를 to(props)로 전달
- 페이지의 Link 모두 찾은 후 모든 페이지(to="경로") Prefetch &rightarrow; 로딩 속도 빠름
